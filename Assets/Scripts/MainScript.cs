	...
	void Update () {
        if (!trialEnded)
        {
            if (!spawned)
            {
                ...
                if (order == 0)
                {
                    sameClone = (GameObject)Instantiate(SameTarget, new Vector2(firstPos, 0.0f), Quaternion.identity);
                    differentClone = (GameObject)Instantiate(DifferentTarget, new Vector2(secondPos, 0.0f), Quaternion.identity);
                }
                else
                {
                    sameClone = (GameObject)Instantiate(SameTarget, new Vector2(secondPos, 0.0f), Quaternion.identity);
                    differentClone = (GameObject)Instantiate(DifferentTarget, new Vector2(firstPos, 0.0f), Quaternion.identity);
                }
            ...
            if (!clip1Played)
            {
                audio.clip = Files[pairIndex[clip1Index]];
                audio.Play();
                clip1Played = true;
            }
            if (clip1Played && !clip2Played && !audio.isPlaying)
            {
                audio.clip = Files[pairIndex[clip2Index]];
                audio.Play();
                clip2Played = true;
            }
